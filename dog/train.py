...
from dog.yolov3 import Create_Yolov3, compute_loss
from dog.utils import load_yolo_weights
from dog.configs import *
...
def train(trainset, testset):
    ...
    if TRAIN_FROM_CHECKPOINT:
        try:
            yolo.load_weights(os.path.join(TRAIN_FROM_CHECKPOINT))
        except ValueError:
    ...
    for epoch in range(TRAIN_EPOCHS):
        ...
        if len(testset) == 0:
            ...
            yolo.save_weights(os.path.join(TRAIN_MODEL_NAME))
            continue
        ...
        if TRAIN_SAVE_BEST_ONLY and best_val_loss>total_val/count:
            ...
            save_directory = os.path.join(TRAIN_CHECKPOINTS_FOLDER,TRAIN_MODEL_NAME+"_val_loss_{:.2f}".format(total_val/count))
            yolo.save_weights(save_directory)
