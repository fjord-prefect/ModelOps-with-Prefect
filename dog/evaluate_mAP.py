...
from dog.utils import load_yolo_weights, detect_image, image_preprocess, postprocess_boxes, nms, read_class_names
...
def get_mAP(Yolo, dataset, score_threshold=0.25, iou_threshold=0.50, TEST_INPUT_SIZE=TEST_INPUT_SIZE):
    ...
    ground_truth_dir_path = 'dog/mAP/ground-truth'
    if os.path.exists(ground_truth_dir_path): shutil.rmtree(ground_truth_dir_path)

    if not os.path.exists('dog/mAP'): os.mkdir('dog/mAP')
    ...
    with open(os.path.join(FEATURE_STORE_PATH,'results.txt'), 'w') as results_file:
